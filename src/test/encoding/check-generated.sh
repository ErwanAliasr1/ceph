...
for type in `./ceph-dencoder list_types`; do
	...
	pids=""
	run_in_background pids bash -c "./ceph-dencoder type $type select_test $n dump_json > $tmp1"
	run_in_background pids bash -c "./ceph-dencoder type $type select_test $n encode decode dump_json > $tmp2"
	run_in_background pids bash -c "./ceph-dencoder type $type select_test $n copy dump_json > $tmp3"
	run_in_background pids bash -c "./ceph-dencoder type $type select_test $n copy_ctor dump_json > $tmp4"
	...
	if [ $deterministic -ne 0 ]; then
	    run_in_background pids bash -c "./ceph-dencoder type $type select_test $n encode export $tmp1"
	    run_in_background pids bash -c "./ceph-dencoder type $type select_test $n encode decode encode export $tmp2"
	    wait_background pids
