...
function corrupt_and_repair_two() {
    ...
    #
    pids=""
    run_in_background pids bash -c "objectstore_tool $dir $first SOMETHING remove"
    run_in_background pids bash -c "objectstore_tool $dir $second SOMETHING remove"
    ...
    #
    pids=""
    run_in_background pids bash -c "objectstore_tool $dir $first SOMETHING list-attrs"
    run_in_background pids bash -c "objectstore_tool $dir $second SOMETHING list-attrs"
    wait_background pids
    return_code=$?
    if [ $return_code -ne 0 ]; then return $return_code; fi
...
function TEST_unfound_erasure_coded() {
    ...
    #
    pids=""
    run_in_background pids bash -c "objectstore_tool $dir $not_primary_first SOMETHING remove"
    run_in_background pids bash -c "objectstore_tool $dir $not_primary_second SOMETHING remove"
    run_in_background pids bash -c "objectstore_tool $dir $not_primary_third SOMETHING remove"
    wait_background pids
    return_code=$?
    if [ $return_code -ne 0 ]; then return $return_code; fi
...
function TEST_list_missing_erasure_coded() {
    ...
    pids=""
    run_in_background pids bash -c "objectstore_tool $dir ${osds[0]} OBJ0 remove"
    run_in_background pids bash -c "objectstore_tool $dir ${osds[1]} OBJ0 remove"
    wait_background pids
    return_code=$?
    if [ $return_code -ne 0 ]; then return $return_code; fi
    ...
    pids=""
    run_in_background pids bash -c "objectstore_tool $dir ${osds[1]} OBJ1 remove"
    run_in_background pids bash -c "objectstore_tool $dir ${osds[2]} OBJ1 remove"
    wait_background pids
    return_code=$?
    if [ $return_code -ne 0 ]; then return $return_code; fi
